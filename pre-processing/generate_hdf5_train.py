...
def generate_hdf5():
    ...
    list_train = anno['list_train']
    
    verb_file = '%s/verb_anno_train.npy' %(file_path)
    verb_anno_train = np.load(verb_file)
    ...
    for i in range(10000, list_train.shape[0]):
        img_name = list_train[i][0][0]
        img_path = '%s/images/train2015/%s' %(file_path, img_name)  
    ...
    with h5py.File(os.getcwd()+ '/train_data_1.h5', 'w') as f:  
        f['data'] = F_imgs.astype(np.float32)  
        f['labels'] = F_labels.astype(np.float32)  
 
    with open(os.getcwd() + '/train_1.txt', 'w') as f:  
        f.write(os.getcwd() + '/train_data_1.h5\n')  
    print i
